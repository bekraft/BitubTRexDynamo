...
public class IfcTransform
{
    ...
    #region Transform delegates

    [IsVisibleInDynamoLibrary(false)]
    public static IfcTransform NewRemovePropertySetsRequest(Logger logInstance, 
        IfcAuthorMetadata newMetadata, 
        string[] removePropertySets, 
        string[] keepPropertySets, 
        bool? caseSensitiveMatching)
    {
    ...
    [IsVisibleInDynamoLibrary(false)]
    public static IfcTransform NewPlacementTransform(Logger logInstance, 
        IfcAuthorMetadata newMetadata, 
        Alignment alignment, 
        object placementStrategy)
    {
        if (!DynamicArgumentDelegation.TryCastEnum(placementStrategy, out ModelPlacementStrategy strategy))
            Log.LogWarning("Unable to cast '{PlacementStrategy}' to type {Type}. Using '{Strategy}'.", 
                placementStrategy, nameof(ModelPlacementStrategy), strategy);
    ...
    [IsVisibleInDynamoLibrary(false)]
    public static IfcTransform NewRepresentationRefactorTransform(Logger logInstance, 
        IfcAuthorMetadata newMetadata, 
        string[] contexts, 
        object refactorStrategy)
    {
        if (!DynamicArgumentDelegation.TryCastEnum(refactorStrategy, out ProductRefactorStrategy strategy))
            Log.LogWarning("Unable to cast '{RefactorStrategy}' to type {Type}. Using '{Strategy}'.", refactorStrategy, nameof(ProductRefactorStrategy), strategy);
    ...
    [IsVisibleInDynamoLibrary(false)]
    public static IfcTransform NewMapConversionTransform(Logger logInstance,
        IfcAuthorMetadata newMetadata,
        MapConversion mapConversionPrefs)
    {
        return new IfcTransform(
            new MapConversionTransform(
                mapConversionPrefs.CrsPrefs!,
                mapConversionPrefs.Prefs!,
                logInstance.LoggerFactory,
                defaultLogFilter)
            {
                EditorCredentials = newMetadata?.MetaData?.ToEditorCredentials()
            });
