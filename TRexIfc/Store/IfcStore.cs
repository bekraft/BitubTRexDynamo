                ...
                qualifier, q => new IfcModel(new IfcStore(source.Store.Logger), qualifier), out var ifcModel))
        {
            if (null != ifcModel)
            {
                ifcModel.Store._modelSupplier = () =>
                {
                    ...
                    {
                        ifcModel.CancelAll();
                        ifcModel.Logger?.LogWarning("Transform of '{0}' to '{1}' has been canceled.",
                            source.CanonicalName(), ifcModel.CanonicalName());
                        return null;
                    }
                    else
                    {
            ...
            {
                GlobalLogging.Instance.Log.Warning("No model found by qualifier {Qualifier}", qualifier);
                return null;
            }
