...
public sealed class Format
{
    ...
    public static Format? FromDynamic(dynamic format)
    {
        ...
        {
            GlobalLogging.Instance.Log.Warning("Not allowed {0}: {1}", format, ane.Message);
        }
        catch (Exception e)
        {
            GlobalLogging.Instance.Log.Warning("Unable to parse '{0}' as format: {1}", format, e.Message);
        }
        return null;
    }
    ...
    {
        if (obj is Format f)
            return f.ID?.Equals(ID) ?? this == obj;
        else
            return false;
    }
